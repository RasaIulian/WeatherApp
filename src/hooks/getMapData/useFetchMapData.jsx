...
const useFetchMapData = (
  ...
  // updated with cloudflare Proxy to avoid CORS issues with RainViewer tiles. The proxy is a simple Cloudflare Worker that fetches the tile and returns it with CORS headers.
  const tileUrl = (path) => {
    const host = hostRef.current.replace(/^https?:\/\//, "");
    const base = `https://${host}${path}/256`;
